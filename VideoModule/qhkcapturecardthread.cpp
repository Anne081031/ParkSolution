...
void QHkCaptureCardThread::ProcessStartPreviewEvent( QCameraEvent* pEvent )
{
    ...
    int nRet = StartVideoPreview( GetChannelHandle( nChannel ), hVideo, &rcClient, TRUE, vdfYUV422Planar, 25 );
    Q_UNUSED( nRet )
}
...
void QHkCaptureCardThread::GetImageFilePath( QString& strFile )
{
    QString strDtDigital;
    QCommonFunction::GetCurrentDateTimeDigital( strDtDigital );
    strFile = strImagePath + strDtDigital + ".jpg";
}
...
void QHkCaptureCardThread::ImageStreamCB( UINT channelNumber, void *context )
{
    ...
    //QString strFile;
    //pThread->GetImageFilePath( strFile );

   // pThread->CaptureStaticImage( strFile, channelNumber );

    QPlateThread* pPlate = pThread->GetPlateThread( );
    pPlate->PostPlateVideoRecognize( byVideo, 704, 576, channelNumber, true );
}
