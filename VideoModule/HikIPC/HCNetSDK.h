...
#define NET_DVR_PLAY_REVERSE    30 //倒放切换为正放
...
#define MINOR_SCREEN_SET_OSD			0x253    /*修改OSD*/
...
#define MINOR_SCREEN_GET_OSD			0x257   /*获取OSD*/
...
#define	MINOR_GET_SCREEN_MODE_CFG		0x289		//获取分屏配置
#define	MINOR_SET_SCREEN_MODE_CFG		0x290		//设置分屏配置
...
#define  DS_65XXD_B10_CLASS			323		//视频综合平台万能解码板 
...
#define  DS_B20_MSU_T               406     //netra主控
...
#define DECODER_ABILITY                     0x261 //解码器能力集
...
{
	DWORD dwSize;
	BYTE sDVRName[NAME_LENGTH];     //DVR名称
...
typedef struct 
{
	...
	BYTE	sPPPoEUser[NAME_LENGTH];							//PPPoE用户名
...
//	BYTE byRes1[328];
	...
	BYTE byFontSize;//字体大小，16*16(中)/8*16(英)，1-32*32(中)/16*32(英)，2-64*64(中)/32*64(英) 
	BYTE byRes[63];
...
typedef struct 
{
	...
	BYTE  byIntervalBPFrame;//0-BBP帧; 1-BP帧; 2-单P帧(2006-08-11 增加单P帧的配置接口，可以改善实时流延时问题)
 	BYTE  byres1;        //保留
 	BYTE  byVideoEncType;   //视频编码类型 0 私有h264;1标准h264; 2标准mpeg4; 3-M-JPEG
 	BYTE  byAudioEncType;   //音频编码类型 0-OggVorbis;1-G711_U;2-G711_A;6-G726
	BYTE  byVideoEncComplexity; //视频编码复杂度，0-低，1-中，2高
    BYTE  byres[9];//这里保留音频的压缩参数
...
typedef struct 
{
	...
	BYTE byRecordType;	//0:定时录像，1:移动侦测，2:报警录像，3:动测|报警，4:动测&报警, 5:命令触发, 6: 智能录像,10-PIR报警，11-无线报警，12-呼救报警，13-移动|报警输入|PIR|无线报警|呼救报警,14-智能交通事件
	char reservedData[3];
...
typedef struct 
{
	...
	BYTE byRecordType;				/* 录象类型 0:定时录像，1:移动侦测，2:报警录像，3:动测|报警，4:动测&报警 5:命令触发, 6: 智能录像,10-PIR报警，11-无线报警，12-呼救报警，13-移动|报警输入|PIR|无线报警|呼救报警,14-智能交通事件*/
	char reservedData;
...
typedef struct 
{
	...
	BYTE sUsername[NAME_LENGTH];		/* 用户名 */
...
{
	DWORD dwSize;
	BYTE sAlarmInName[NAME_LENGTH];	/* 名称 */
...
typedef struct 
{
	DWORD dwAlarmType;/*0-信号量报警,1-硬盘满,2-信号丢失,3－移动侦测,4－硬盘未格式化,5-读写硬盘出错,6-遮挡报警,7-制式不匹配, 8-非法访问, 9-视频信号异常，
                        10-录像异常 11- 智能场景变化  12-阵列异常 13-前端/录像分辨率不匹配*/
...
typedef  struct tagNET_DVR_IPSERVER_STREAM
{
    ...
    BYTE    byDVRName[NAME_LENGTH];    // DVR名称
    WORD    wDVRSerialLen;          // 序列号长度
    WORD    byRes1[2];              // 保留字节
    BYTE    byDVRSerialNumber[SERIALNO_LEN];    // DVR序列号长度
    BYTE    byUserName[NAME_LENGTH];   // DVR 登陆用户名
...
typedef struct tagNET_DVR_DEV_CHAN_INFO
{
    ...
    BYTE	byRes[2];
    BYTE    byDomain[MAX_DOMAIN_NAME];	//设备域名
    BYTE	sUserName[NAME_LENGTH];	//监控主机登陆帐号
...
typedef struct tagNET_DVR_DDNS_STREAM_CFG
{
    ...
    BYTE   sUserName[NAME_LENGTH];   // DVR 登陆用户名
...
typedef struct tagNET_DVR_HKDDNS_STREAM
{
    BYTE    byEnable;				 // 是否在线
    BYTE    byRes[3];               // 保留字节
    BYTE    byDDNSDomain[64];		// hkDDNS服务器
    WORD    wPort;                  // hkDDNS 端口
    WORD    wAliasLen;              // 别名长度
    BYTE    byAlias[NAME_LENGTH];		 // 别名
    WORD    wDVRSerialLen;          // 序列号长度
    BYTE    byRes1[2];              // 保留字节
...
typedef union tagNET_DVR_GET_STREAM_UNION
{
	...
	NET_DVR_HKDDNS_STREAM	struHkDDNSStream;   //通过hkDDNS去取流
...
{
	DWORD dwSize;
	BYTE sAlarmOutName[NAME_LENGTH];	/* 名称 */
...
typedef struct tagNET_DVR_ALARMHOST_LOG_RET
{
    ...
    BYTE		    sUserName[NAME_LENGTH];     // 操作用户
...
typedef struct 
{
	...
	union
	{
		...
		{
			DWORD dwChannel;
			BYTE sUserName[NAME_LENGTH];	/*请求视频用户名*/
			BYTE sPassword[PASSWD_LEN];	/* 密码 */
...
{		
	DWORD		dwSize;
	BYTE		sAccount[NAME_LENGTH];				/* 账号*/ 
	BYTE		sPassword[MAX_EMAIL_PWD_LEN];			/*密码 */
	struct
	{
		BYTE	sName[NAME_LENGTH];				/* 发件人姓名 */
...
typedef struct 
{
	...
	仅支持当前分辨率的抓图*/
	WORD	wPicSize;				/* 可以通过能力集获取, 0-CIF，1-QCIF，2-D1，3-UXGA(1600x1200)，
...
typedef struct 
{
	BYTE sUsername[NAME_LENGTH];  /* DDNS账号用户名/密码 */
...
typedef struct
{
	...
	BYTE sUsername[NAME_LENGTH];			/* DDNS用户名*/
...
{
    BYTE byEnableDDNS;
    BYTE byHostIndex;/*0-IPServer 1－Dyndns 2－PeanutHull(花生壳)，3- NO-IP, 4- hkDDNS*/
    BYTE byRes1[2];
    struct
    {    
        BYTE sUsername[NAME_LENGTH];			/* DDNS账号用户名*/
        BYTE sPassword[PASSWD_LEN];			/* 密码 */
        BYTE sDomainName[MAX_DOMAIN_NAME];	/* 设备配备的域名地址 */
...
{					
    ...
    BYTE	byRes2;						/* 保留 */
	TTY_CONFIG RemoteSerialDevCfg;
	BYTE  	byUsername[NAME_LENGTH]; 		/* 32BYTES */
...
typedef struct tagNET_DVR_SUBSYSTEMINFO
{
    ...
    NET_DVR_IPADDR	struGatewayIpAddr;	/* 网关地址*/   
    BYTE		sUserName[NAME_LENGTH];	/* 用户名 （此参数只能获取）*/
...
typedef struct
{
	...
	//bySupport1 & 0x40, 延时预览
	//bySupport1 & 0x80, 表示是否支持车牌新报警信息2012-9-28
    BYTE bySupport2; //能力集扩展，位与结果为0表示不支持，1表示支持
	//bySupport2 & 0x1, 表示解码器是否支持通过URL取流解码
	WORD wDevType;              //设备型号
	BYTE byMultiStream;//支持的多码流,按位表示,0-不支持,1-支持,bit0-码流3,bit1-码流4,依次类推
	BYTE byMultiStreamProto;//多码流是否支持rtsp协议取流,按位表示,0-不支持,1-支持,bit0-码流3,bit1-码流4,依次类推
    BYTE byRes2[14];		//保留    
...
{   
    ...
    char sDeviceName[NAME_LENGTH];		    /* 设备名字 */
...
typedef struct 
{
    ...
    DWORD dwFileType;//录象文件类型
	   //不带卡号，0xff－全部，0－定时录像,1-移动侦测 ，2－报警触发，3-报警|移动侦测 4-报警&移动侦测 5-命令触发 6-手动录像,7-智能录像，10-PIR报警，11-无线报警，12-呼救报警，13-移动|报警输入|PIR|无线报警|呼救报警
	 //带卡号，0xff－全部，0－定时录像，1-移动侦测，2－接近报警，3－出钞报警，4－进钞报警，5-命令触发，6－手动录像，7－震动报警，8-环境报警，9-智能报警，10-PIR报警，11-无线报警，12-呼救报警，13-移动|报警输入|PIR|无线报警|呼救报警
		...
		struct 
		{
			...
			union
            {
				struct 
				{
                    ...
                    BYTE byRes1[2];
                    BYTE byAnonyIdentity [NAME_LENGTH]; //匿名身份
					BYTE byUserName[NAME_LENGTH]; //用户名
                    BYTE byPassword[NAME_LENGTH]; //密码
                    BYTE byRes[44];
				...
				struct 
				{
                    BYTE byEapolVersion; //EAPOL版本，0-版本1，1-版本2
                    BYTE byRes1[3]; 
                    BYTE byIdentity[NAME_LENGTH]; //身份
					BYTE byPrivateKeyPswd[NAME_LENGTH]; //私钥密码
                    BYTE byRes[76]; 
				}EAP_TLS; 
            }auth_param;
...
{
    WORD wDecibel;        //声音强度(保留)
	BYTE bySensitivity;    /* 灵敏度参数，范围[1,5] */
    BYTE byAudioMode;    //声音检测模式，0：启用灵敏度检测；1：启用分贝阈值检测
    BYTE byRes[56];    //保留    
...
typedef struct tagNET_VCA_ONE_RULE
{
    ...
    BYTE   byRuleName[NAME_LENGTH];		//规则名称
...
typedef enum _MAIN_EVENT_TYPE_
{
	...
	EVENT_AUX_ALARM = 4, //辅助报警 add by wxm 12-12-17 for IPC V5.0
...
typedef struct tagNET_DVR_SEARCH_EVENT_PARAM
{
	...
	union	
	{
		...
		struct//辅助报警
		{
			BYTE byChanNo[MAX_CHANNUM_V30];	//触发辅助报警的通道
			BYTE byRes[SEARCH_EVENT_INFO_LEN - MAX_CHANNUM_V30];
		}struAuxAlarmParam;
	}uSeniorParam;
...
typedef struct tagNET_DVR_SEARCH_EVENT_RET
{
	...
	union		
	{
		...
		struct //流id录像查询结果
		{
			...
			BYTE  byRes1[2];
			BYTE  byFileName[NAME_LENGTH]; 	//文件名
			BYTE  byRes[260];
		}struStreamIDRet;
	}uSeniorRet;
...
typedef struct    tagNET_DVR_PTZ_POSITION
{
    ...
    BYTE byEnable;
    BYTE byRes1[3];  //保留
    BYTE byPtzPositionName[NAME_LENGTH]; //场景位置名称
...
typedef struct tagNET_DVR_ONE_LANE
{
	...
	BYTE	byRes1[11];	// 保留字节
	BYTE    byLaneName[NAME_LENGTH];       // 车道规则名称
...
{
    BYTE    byRuleName[NAME_LENGTH];                 //车道规则名称 
...
{
    BYTE byWhiteBalanceMode; /*0手动白平衡; 1自动白平衡1（范围小）; 2 自动白平衡2（范围宽，2200K-15000K）;3锁定白平衡，4-白炽灯，5-暖光灯，6-自然光，7-日光灯*/
...
typedef struct tagNET_DVR_CAMERAPARAMCFG
{
	...
	BYTE bySceneMode;  //场景模式：0-室外，1-室内
...
typedef struct tagNET_DVR_VEHICLE_INFO_
{
	...
	BYTE  byIllegalType;           //违规类型，0-正常;1-低速2-超速3-逆行，,4-闯红灯,5-压车道线,6-不按导向，7-路口滞留，8-机占非，9-违法变道
    BYTE  byRes2[3]; //保留
...
typedef struct tagNET_DVR_SNMPCFG
{
    ...
    BYTE    byReadCommunity[NAME_LENGTH]; //读共同体，最多31,默认"public"
    BYTE    byWriteCommunity[NAME_LENGTH];//写共同体,最多31 字节,默认 "private"
...
typedef struct tagNET_DVR_PHY_DISK_INFO
{
    ...
    BYTE    byStatus;       // 硬盘状态；  0-正常 1-异常 0xff-不存在
...
typedef struct tagNET_DVR_ARRAY_INFO
{
    ...
    BYTE    byStatus;  // 0-在线 1-磁盘丢失 2-下线 3-降级 4-异常 5-次正常 6-外来盘  0xff-不存在
...
{
    WORD    wSlot;    //虚拟磁盘槽位  
    BYTE    byStatus; // 状态0-正常 1-降级 2-已删除 3-磁盘丢失 4-下线 5-次正常 6-外来 7-异常 0xff-不存在
...
typedef struct  tagNET_DVR_CODESPLITTERASSOCIATE
{
	...
	WORD		wPort;        //编码器端口号
	BYTE		byRes1[6];
    BYTE		sUserName[NAME_LENGTH];	/* 用户名（32） */
...
{
	char sDeviceID[NAME_LENGTH];//设备ID串
...
{
    BYTE byDeviceID[NAME_LENGTH];/*设备的ID串*/
...
{
	BYTE byDialNum[NAME_LENGTH];		/* dial number, eg. "#777" */
	BYTE byUserName[NAME_LENGTH];	/* user name, eg. "card" */
	BYTE byPassword[DIALPASSWD_LEN];	/* password, eg. "card" */
	BYTE byApn[NAME_LENGTH];		/* access point name, eg. "cmnet" */
...
typedef struct tagNET_DVR_ALARMIN_PARAM
{
	...
	BYTE	byModuleType;	//模块类型，1-本地防区，2-单防区，3-双防区，4-8防区，5-8路模拟量防区
...
typedef struct tagNET_DVR_ALARMOUT_PARAM
{
	...
	BYTE    byAssociateAlarmIn[MAX_ALARMHOST_ALARMIN_NUM];   //表示警号跟随的报警输入通道 （多个报警输入同时触发一个警号输出）数组下标0表示报警输入1，依次类推 0-不跟随 1-跟随
	BYTE	byModuleType;	//外接触发器类型，1-本地触发器， 2-4路触发器，3-8路触发器
...
typedef struct tagNET_DVR_ALARMHOST_ABILITY
{
	...
	DWORD	dwFaultType;			//设备支持的故障类型，bit0-交流电断电，bit1-蓄电池欠压，bit2-主机防拆开，bit3-电话线掉线，bit4-主键盘掉线，bit5-网络故障，bit6-GPRS异常，bit7-MBUS异常，bit8-硬盘异常	
...
{
	DWORD   dwSize;                 // 结构体大小
	BYTE    sDeviceName[NAME_LENGTH];  // 前端设备名称 
...
typedef struct tagNET_DVR_SENSOR_ALARM
{
    ...
    BYTE    byName[NAME_LENGTH]; 	// sensor 名称
...
{
	DWORD 		dwSize;
    BYTE		sLEDName[NAME_LENGTH/*32*/];	// LED屏幕名称
...
typedef struct tagNET_DVR_DISP_CHAN_INFO
{
    ...
    BYTE	bySlotNum;//槽位号
    BYTE	byRes[7];
    BYTE	sUserName[NAME_LENGTH];	/*登陆帐号 */
    BYTE	sPassword[PASSWD_LEN];	/*密码 */
...
typedef struct tagNET_DVR_SUBSYSTEMINFO_V40
{
	...
	BYTE		byInterfaceType;//接口类型，1-BNC，2-VGA，3-HDMI，4-DVI，5-SDI，6-光纤，0xff表示无效
...
typedef struct tagNET_DVR_SUBSYSTEM_ABILITY
{
	...
	union
	{
		...
		struct
		{
            ...
            BYTE  byRes1[36];
            BYTE  byBelongBoard;//所属子板号
            BYTE  byDispChanNums ;//显示输出个数
            BYTE  byDsipStartChan;//显示输出号在子板中的起始号
			BYTE	byDecoderType ; //解码板类型  0-普通解码板 1-万能解码板
            BYTE  byRes2[152];
        ...
        struct
        {
            ...
            BYTE	byBelongBoard ;//所属子板号
            BYTE	byFiberStartChan;//光纤在子板中的起始号
			BYTE    byRes[195];
...
typedef struct tagNET_DVR_VIDEOPLATFORM_ABILITY_V40
{
    ...
    BYTE    bySupportLayerNums;//所支持的图层数
    BYTE    byNotSupportPreview;//是否支持预览,1-不支持，0-支持
    BYTE    byNotSupportStorage;//是否支持存储,1-不支持，0-支持
...
typedef struct tagNET_DVR_EVENT_CAPTURE
{
	...
	//	6-10m 7-30m 8-1h 9-12h 10-24h
    NET_DVR_REL_CAPTURE_CHAN struRelCaptureChan[MAX_PIC_EVENT_NUM]; // 数组下标 0 移动侦测触发抓图 1 视频遮挡触发抓图,2 视频丢失触发抓图,数组3表示PIR报警抓图，数组4表示无线报警抓图，数组5表示呼救报警抓图
...
typedef struct tagNET_DVR_FIND_PICTURE_PARAM
{
    DWORD  dwSize;         // 结构体大小 
    LONG   lChannel;       // 通道号
    BYTE   byFileType;     // 查找的图片类型:0定时抓图1 移动侦测抓图 2 报警抓图3  报警 | 移动侦测抓图 4 报警 & 移动侦测抓图 6 手动抓图  10- PIR报警，11- 无线报警，12- 呼救报警，0xa 预览时截图0xff全部类型
...
{	
	DWORD					dwSize;				//结构体大小
	WORD					wEffectMode;        //模式 0-标准 1-室内 2-室外 3-弱光 255-自定义
	BYTE					byRes1[146];        //保留
...
{
	WORD	wDeviceType;	//数据采集设备型号:1:米乐 2:镭彩 3:优力
...
typedef struct tagNET_DVR_INQUEST_SENSOR_INFO
{
	...
	DWORD   dwSupportPro;      //支持协议类型,按位表示
...
{
	DWORD  dwRecordMode;         //刻录模式:1 单室双刻模式 2 单室轮刻模式 3 双室双刻模式
    DWORD  dwWorkMode;           //工作模式:0 标准模式 1 通用模式
	DWORD  dwResolutionMode;     //设备分辨率，0:标清 1:D1 2:720P 3:1080P
...
{
	DOWNLOAD_CERT = 0,
	DOWNLOAD_ALARM_RECORD = 1 //下载报警录像
...
typedef struct tagNET_DVR_SOCKS_CFG
{
    ...
    BYTE 			byPassword[NAME_LENGTH];			// 密码SOCKS5才用
...
typedef struct tagNET_DVR_CERT_INFO
{
	...
	BYTE     			byPasswd[NAME_LENGTH];   //私钥密码
	...
	BYTE     			serialNumber[NAME_LENGTH];   //证书标识码（自签名证书信息获取时有效）
	BYTE 				byVersion; 
...
{
	BYTE byValid;       //是否有效
	BYTE byRecord;      //(只读)录像类型, 按位表示:0: 不在录像；1：在录像
	WORD wChannelNO;   //通道号
...
{  
    ...
    BYTE  sCamName[NAME_LENGTH]; /*cam的名称*/
    ...
    BYTE  sPurpose[NAME_LENGTH];/*用途描述*/
	BYTE  byRes2[20];  
...
typedef struct tagNET_MATRIX_MATRIXINFO 
{
	...
	BYTE	sUserName[NAME_LENGTH];   /*登录用户名*/
...
typedef struct tagNET_VCA_HUMAN_ATTRIBUTE
{
	...
	BYTE   byName[NAME_LENGTH]; //姓名
	NET_DVR_AREAINFOCFG struNativePlace; //籍贯参数
	BYTE   byCertificateNumber[NAME_LENGTH];  //证件号
	BYTE   byRes2[20];
...
typedef struct tagNET_VCA_BLACKLIST_INFO
{
	...
	BYTE   byRemark[NAME_LENGTH]; //备注信息
	BYTE   byRes2[20];
...
typedef struct tagNET_VCA_SINGLE_PATH
{
	...
	BYTE  byType;   //0-存储抓拍，1-存储黑名单比对报警，2-存储抓拍和黑名单比对报警
	BYTE  byRes1[6]; //保留
...
typedef struct tagNET_DVR_DEV_ACCESS_CFG
{
	...
	BYTE 	byRes1;				//保留
    BYTE	sUserName[NAME_LENGTH];	//接入设备的登录帐号
...
typedef struct tagNET_DVR_LAYOUTCFG
{
    ...
    BYTE  byValid;								//布局是否有效
    BYTE  byRes1[3];
    BYTE  byLayoutName[NAME_LENGTH/*32*/];			//布局名称
...
typedef struct tagNET_DVR_INPUTSTREAMCFG
{
    ...
    BYTE	sCamName[NAME_LENGTH] ;			//信号输入源名称
    ...
    BYTE	sGroupName[NAME_LENGTH/*32*/];	//网络信号源分组 组名
...
typedef struct tagNET_DVR_SCREENCONTROL_ABILITY
{
	...
	BYTE byPlanNum;    		/*预案个数*/
    BYTE byIsSupportOSD;	/*是否支持OSD*/
    BYTE byIsSupportLogo;   /*是否支持logo*/
    BYTE byIsSupportPlan;   /*是否支预案*/
    BYTE byIsSupportGroup;  /*是否支分组*/
    BYTE byIsSupportLayout; /*是否支布局管理*/
...
{
	DWORD	dwSize;
    BYTE	byAlarmType;	//报警类型，1-子板拔出，2-子板插入，3-子系统状态异常，4-子系统恢复恢复 5-输入源异常   6-温度报警 7-FPGA版本不匹配 8-预案开始 9-预案结束
	BYTE	byBoardType;	// 1-输入板 2-输出板 
...
typedef struct tagNET_DVR_SCREEN_SCREENINFO
{
    ...
    BYTE sUserName[NAME_LENGTH];	/*登录用户名*/
    BYTE sPassword[PASSWD_LEN]; /*登录密码*/   
	BYTE sDevName[NAME_LENGTH];	/*设备名称*/
...
{	
    ...
    BYTE res[2];
	WORD wSour_width;			/* 原图片的宽度 */
	WORD wSour_height;			/* 原图片的高度 */
...
typedef struct tagNET_DVR_OSDCFG
{
    ...
    BYTE    uByFontColor_y; /*字体颜色Y    0-255*/
    BYTE    uByFontColor_u; /*字体颜色U    0-255*/
    BYTE    uByFontColor_v; /*字体颜色V    0-255*/
    BYTE    uByBackColor_y; /*背景颜色Y    0-255*/
    BYTE    uByBackColor_u; /*背景颜色U    0-255*/
	BYTE    uByBackColor_v; /*背景颜色V    0-255*/
...
typedef struct tagNET_DVR_PLAN_CFG
{
	...
	BYTE	byRes1[2];
    BYTE  	byPlanName[NAME_LENGTH/*32*/]; //预案名称
...
{
	DWORD	dwSize;
	BYTE	sDeviceID[NAME_LENGTH]; //被控设备的设备ID
	WORD	wChan;				 //被控通道
...
{
	DWORD dwSize;
	BYTE byDehazeMode; //0-不启用，1-自适应模式
	BYTE byRes[7]; 
...
typedef struct
{
	NET_DVR_UPNP_PORT_STATE strUpnpPort[12];     //端口映射状态,数组0 web server端口 数组1 管理端口 数组2 rtsp端口
	BYTE   byRes[200];              //保留
...
{
    BYTE		byJoinStatus;//关联状态，0-没有关联，1-已经关联，2-被占用
...
typedef struct tagNET_DVR_PRINTER_ENABLE
{
	...
	BYTE	byRes1[2];
	DWORD	dwAlarmInfo;			//报警信息，按位表示。bit0-防区报警，bit1-紧急报警，bit-2挟持
	DWORD	dwDeviceInfo;			//设备信息，按位表示。bit0-交流电、bit1-蓄电池、bit2-电话线、bit3-测试报告、bit4-防拆、bit5-键盘掉线、bit6-SIM卡、bit7-GPRS/3G、bit8-有线网络、bit9-扩展总线异常
	DWORD	dwOperateInfo;		//操作信息，按位表示。bit0-布防、bit1-撤防、bit2-消警、bit3-旁路、bit4-进入编程、bit5-退出编程、bit6-主机复位
...
typedef struct tagNET_DVR_ALARMHOST_WIRELESS_NETWORK_CFG
{
	...
	BYTE	byRes1;
    BYTE	bySIMNum[NAME_LENGTH/*32*/]; //SIM卡号（手机号）
...
typedef struct tagNET_DVR_PREVIEWINFO
{
	...
	DWORD dwLinkMode;// 0：TCP方式,1：UDP方式,2：多播方式,3 - RTP方式，-RTP/RTSP,5-RSTP/HTTP 
	HWND hPlayWnd;//播放窗口的句柄,为NULL表示不播放图象
	BOOL bBlocked;  //0-非阻塞取流, 1-阻塞取流, 如果阻塞SDK内部connect失败将会有5s的超时才能够返回,不适合于轮询取流操作.
...
typedef struct tagNET_DVR_TRIGGER_EVENT
{
	...
	DWORD	dwSubSystemEventTriggerAlarmoutOn[MAX_ALARMHOST_SUBSYSTEM/*32*/]; //子系统事件触发报警输出开启,dwSubSystemEvent[0]表示子系统1，dwSubSystemEvent[2]表示子系统2，具体的子系统个数从能力集获取（wSubSystem），每个子系统的事件按位表示，bit0-进入延时，bit1-退出延时，bit2-布防，bit3-撤防，bit4-报警，bit5-消除报警记忆
	DWORD	dwSubSystemEventTriggerAlarmoutOff[MAX_ALARMHOST_SUBSYSTEM/*32*/]; //子系统事件触发报警输出关闭,wSubSystemEvent[0]表示子系统1，dwSubSystemEvent[2]表示子系统2，具体的子系统个数从能力集获取（wSubSystem），每个子系统的事件按位表示，bit0-进入延时，bit1-退出延时，bit2-布防，bit3-撤防，bit4-报警，bit5-消除报警记忆
	BYTE	byRes[128];
...
{
	DWORD 	dwSize;
	DWORD	dwCheckFault;	//是否检测故障，bit0-交流电断电，bit1-蓄电池欠压，bit2-主机防拆开，bit3-电话线掉线，bit4-主键盘掉线，bit5-网络故障，bit6-GPRS异常，bit7-MBUS异常，bit8-硬盘异常，具体支持的故障类型从能力集获取
	DWORD	dwOverallFaultJointLED;		//故障关联全局键盘指示灯输出 bit0-交流电断电，bit1-蓄电池欠压，bit2-主机防拆开，bit3-电话线掉线，bit4-主键盘掉线，bit5-网络故障，bit6-GPRS异常，bit7-MBUS异常，bit8-硬盘异常，具体支持的故障类型从能力集获取
	DWORD	dwOverallFaultJointSound;	//故障关联全局键盘声音输出 bit0-交流电断电，bit1-蓄电池欠压，bit2-主机防拆开，bit3-电话线掉线，bit4-主键盘掉线，bit5-网络故障，bit6-GPRS异常，bit7-MBUS异常，bit8-硬盘异常，具体支持的故障类型从能力集获取
	DWORD	dwSubSystemFaultJointLED[MAX_ALARMHOST_SUBSYSTEM]; /*故障关联子系统键盘指示灯输出，dwSubSystemFaultJointLED[0]表示子系统1的关联信息。dwSubSystemFaultJointLED[0]的每一位表示一中故障，bit0-交流电断电，bit1-蓄电池欠压，bit2-主机防拆开，bit3-电话线掉线，bit4-主键盘掉线，bit5-网络故障，bit6-GPRS异常，bit7-MBUS异常，bit8-硬盘异常; dwSubSystemFaultJointLED[1]表示子系统2，以此类推*/
	DWORD	dwSubSystemFaultJointSound[MAX_ALARMHOST_SUBSYSTEM];/*故障关联子系统键盘声音输出（声音在键盘上输出）dwSubSystemFaultJointLED[0]表示子系统1的关联信息。dwSubSystemFaultJointSound [0]的每一位表示一中故障，bit0-交流电断电，bit1-蓄电池欠压，bit2-主机防拆开，bit3-电话线掉线，bit4-主键盘掉线，bit5-网络故障，bit6-GPRS异常，bit7-MBUS异常，bit8-硬盘异常; dwSubSystemFaultJointSound [1]表示子系统2，以此类推*/
	BYTE	byRes[64];
...
typedef struct tagNET_DVR_CID_ALARM
{
    ...
    BYTE	sCIDDescribe[NAME_LENGTH/*32*/];	//CID事件名
    ...
    BYTE	sUserName[NAME_LENGTH/*32*/];		//网络用户用户名
...
{
	DWORD dwSize;
	BYTE	byModuleType;	//1-防区，2-触发器      
	BYTE	byZoneType;		//防区类型，1-本地防区，2-单防区，3-双防区，4-8防区，5-8路模拟量防区，0xff表示该参数无效
	BYTE	byTriggerType;	//触发器类型，1-本地触发器， 2-4路触发器，3-8路触发器，0xff表示该参数无效
	BYTE	byRes[253];
...
{
	DWORD           dwSize;
	BYTE			byRecord;         //(只读)录像类型, 0：不在录像，1：在录像
	BYTE			byRes1[3];        //保留字节
	DWORD			dwRelatedHD;      //关联磁盘
	BYTE			byRes2[8];        //保留字节
...
{
    BYTE		byEnable;					//是否启用
	BYTE		byProType;					//协议类型，0-海康协议(default), (需要从设备获取能力)
...
{
    NET_ITC_PLATE_RECOG_PARAM struPlateRecog; //牌识参数
    NET_ITC_SINGLEIO_PARAM struSingleIO[MAX_IOIN_NUM]; //单个IO触发参数
    BYTE  byRes[32];
...
{
	BYTE    byRadarType;	//雷达类型，0-无雷达,1-安道雷雷达，2-奥利维亚，3-川速微波，0xff-其它类型
...
typedef struct tagNET_VCA_DATABASE_PARAM
{
	...
	BYTE byDataBaseName[NAME_LENGTH]; //数据库名称
...
typedef struct tagNET_VCA_INQUIRE_SNAPDB_COND
{
	...
	BYTE byStartBirthDate[MAX_HUMAN_BIRTHDATE_LEN]; //起始出生年月，如：201106
	BYTE byEndBirthDate[MAX_HUMAN_BIRTHDATE_LEN]; //截止出生年月，如201106
	BYTE byAttribute1[NAME_LENGTH]; //属性1
	BYTE byAttribute2[NAME_LENGTH]; //属性2
	BYTE byRes[12]; //保留
...
{ 
	...
	BYTE  byRemark1[NAME_LENGTH]; //备注信息1
...
{
    BYTE byAlarmName[NAME_LENGTH];	/* 报警名称 */
...
typedef struct tagNET_DVR_PREVIEW_DISPLAYCFG
{
	...
	BYTE byDisplayMode; //显示模式，详见PREVIEW_DISPLAY_MODE
	BYTE byRes[61];
}NET_DVR_PREVIEW_DISPLAYCFG, *LPNET_DVR_PREVIEW_DISPLAYCFG;

typedef enum _PREVIEW_DISPLAY_MODE_
{
    PREVIEW_DISPLAY_NONE = 0, //无
		PREVIEW_DISPLAY_FISHEYE = 1, //单窗口鱼眼
		PREVIEW_DISPLAY_PTZ = 2,   //单窗口PTZ
		PREVIEW_DISPLAY_FULL_PANORAMA = 3, //单窗口四角全景展开
		PREVIEW_DISPLAY_TWO_AROUNDPANORAMA = 4,  //两个窗口180°（around panorama） 上下分屏
		PREVIEW_DISPLAY_WIDEPANORAMA_TWO_PTZ = 5, //一个360°（around panorama） 加上两个PTZ分屏；三个窗口
		PREVIEW_DISPLAY_ONE_PANORAMA_TWO_PTZ = 6, //四角全景（局部）+2 PTZ分屏
		PREVIEW_DISPLAY_FISHEYE_THREE_PTZ = 7 //一个球加上三个PTZ分屏（四画面）
...
typedef struct tagNET_DVR_DECODER_JOINT_PARAM
{
	...
	BYTE	sDeviceName[NAME_LENGTH];	//被关联解码器的设备名称
	BYTE	sChanName[NAME_LENGTH];	//被关联解码器的解码通道名称
	BYTE	byRes2[32];		// 保留字节
...
{
	DWORD	dwUserID;
	BYTE	sDeviceID[16];	//设备ID
	LONG	nPicViewHandle; //回显句柄
	WORD	wSignalIndex;	//信号源索引
	WORD	wHeadLen;		//文件头长度 海康的码流文件头都是40
...
typedef struct tagNET_DVR_CHAN_INFO
{
	...
	BYTE   byUsed;           /*是否使用,0-没有使用，1-在使用*/
	BYTE   byRes1[2];
	BYTE   sChanName[NAME_LENGTH];    /*通道名称*/
...
{
	DWORD	dwSize;
    BYTE	byDevName[NAME_LENGTH/*32*/];//网络信号源名称
	NET_DVR_PU_STREAM_CFG struPuStream;  //网络信号源信息，当流媒体结构体struStreamMediaSvrCfg中的byValid为0时，直接使用struDevChanInfo中的参数作为网络信号源参数
	BYTE    byValid;			//网络信号源是否有效，0-无效，删除网络信号源时设为0  1-有效，添加和修改网络信号源时设为1/
	BYTE 	byRes1[3];	//保留
    BYTE	sGroupName[NAME_LENGTH/*32*/];//网络信号源所在组的组名，网络信号源根据这个组名来分类
    BYTE	byRes2[28];	//保留		
...
typedef struct  tagNET_DVR_SINGLE_DEV_INFO 
{
	...
	BYTE	byRes1;
	BYTE	sDeviceID[DVCS_DEVICEID_LEN];    /*设备ID标识符*/ 
	BYTE	sDevName[NAME_LENGTH];     /*设备名称*/
...
{
	DWORD dwSize;
	BYTE  byShowText;		// 是否显示文字，0-显示文字，1-不显示文字
	BYTE  byRes[35];
...
//
//大屏控制器图片回显数据类型，现在海康大屏控制器回显的是图片（DATA_BMP），分布式大屏回显的是（DATA_SUBSTREAM）
typedef enum
{
	NET_DVR_DATA_BMP = 0,
...
{
	DWORD	dwSize;
	DWORD	dwSignalIndex;	//信号源序号，该参数用于海康大屏控制器，其他设备该值设为0
	BYTE	sDeviceID[DVCS_DEVICEID_LEN];	//设备ID
	BYTE	byChanIndex;	//通道号
	BYTE    byRes1[3];
	//视频综合平台回显参数
...
{
	DWORD  dwSize;
	BYTE   byAlarmType;                /* 报警类型 1-设备上线，2-设备离线 3-服务器上线 4-服务器离线*/
	BYTE   byDeviceType;				/* 设备类型(只能获取) */ 
	BYTE   byRes1[2];
	BYTE   sDeviceID[DVCS_DEVICEID_LEN];
	BYTE   byRes2[80];
...
{
	BYTE				byFileName[NAME_LENGTH]; 	// 录像文件名，老协议只传32个文件名
    DWORD				dwLockDuration;			// 锁定持续时间,单位秒，0xffffffff表示永久锁定
	BYTE				byRes[512];
...
typedef struct tagNET_DVR_FTP_CFG_V40
{
	...
	BYTE 				bySubDirMode;       /*二级目录，0x1=使用通道名,0x2=使用通道号
											   0x3=使用时间(年月日),0x4=使用车道号 0x5-使用自定义*/
...
{
	DWORD dwSize;
	BYTE  byTransparency; //使能透明度，0-关，非0-关	
...
{
	DWORD dwSize;
	BYTE  sSceneName[NAME_LENGTH];//场景名称
	BYTE  byRes [80];
...
typedef struct tagNET_DVR_EXTERNAL_MATRIX_CFG
{
	...
	BYTE		byRes1[3];
    BYTE	    sMatrixName[NAME_LENGTH/*32*/];       // 矩阵的名称
...
{
	DWORD   dwSize;
	BYTE	sUserName[NAME_LENGTH];			// 用户名，最大16字节
	BYTE	sPassWord[PASSWD_LEN];		// 密码
	NET_DVR_IPADDR	struUserIP;	// 用户IP地址(为0时表示允许任何地址)
	BYTE	byMacAddr[MACADDR_LEN];			// 物理地址
...
{
	DWORD								dwSize;
    BYTE								byStreamMode;//取流模式，0-无效，1-通过IP或域名取流，2-通过URL取流
	BYTE								byRes1[3];
...
typedef struct tagNET_DVR_SAFETYCABIN_PERSON_SIGNAL_CFG
{
	...
	BYTE  byDevUseTimeout; //防护舱使用超时时间，单位：分,范围：0-30，默认超时15min,超时后通过布防通道上传报警
	BYTE  byRes1;
...
typedef struct tagNET_DVR_CAMERA_SETUPCFG
{
	...
	BYTE byLensType; //镜头焦距类型，0-8mm,1-12mm,2-16mm,3-25mm,4-35mm,5-50mm
    BYTE byRes[61]; 
