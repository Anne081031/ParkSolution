...
QPlateThread::QPlateThread(QObject *parent) :
    QBaseThread( "QPlateThread", parent )
{
...
bool QPlateThread::ThreadInitialize( )
{
    return QBaseThread::ThreadInitialize( );
}

void QPlateThread::ThreadUninitialize( )
{
...
void QPlateThread::PostPlateVideoRecognize( QByteArray &byVideo, int nWidth,
                                            int nHeight, int nChannel, bool bMultiThread )
{
...
void QPlateThread::PostEvent( QEvent *pEvent )
{
...
BOOL QPlateThread::InitVZSDK( int nFormat, qint32 nChannel )
{
    ...
    QString strLog = "%1 Format %2 Chhanel %3";

    if ( bDongleOneWay && 1 != nChannel ) {
        strLog = strLog.arg( "设置为单路加密狗，但超一路初始化", QString::number( nFormat), QString::number( nChannel ) );
        EmitLog( strLog );
    ...
    if ( !bRet ) {
        strLog = strLog.arg( "LPR_SetImageFormat 失败", QString::number( nFormat), QString::number( nChannel ) );
        EmitLog( strLog );
    ...
    if ( !bRet ) {
        strLog = strLog.arg( "LPR_InitEx 失败", QString::number( nFormat), QString::number( nChannel ) );
    ...
    if ( !bRet ) {
        strLog = strLog.arg( "LPR_SetPlateType 失败", QString::number( nFormat), QString::number( nChannel ) );
    ...
    if ( !bRet ) {
        strLog = strLog.arg( "LPR_SetSpecialParameters 失败", QString::number( nFormat), QString::number( nChannel ) );
    }
...
{
    if ( bDongleOneWay && 1 != nChannel ) {
        QString  strLog = "设置为单路加密狗，但超一路释放初始化";
        EmitLog( strLog );
        return;
    }
